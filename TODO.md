...
- [ ] Continue the broad `openssl_test` source-glob experiment with preparation temporarily bypassed. Use the deliberately broad `crypto/ec/ecp_nist*.c` exclusion during source-selection debugging, including the configured generic/NISTZ256 sources as well as the disabled GCC-128 group and the perlasm table input.
