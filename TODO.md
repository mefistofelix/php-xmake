...
- [ ] Continue the broad `openssl_test` source-glob experiment with preparation temporarily bypassed. Use the deliberately broad `crypto/ec/ecp_nist*.c` exclusion, exclude `**/*md2*.c` for the default-disabled MD2 closure, and exclude the six non-x64 `crypto/*cap.c` implementations while retaining `cpuid.c` plus generated `x86_64cpuid.asm`.
