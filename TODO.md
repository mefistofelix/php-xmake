...
- [ ] Integrate the next XML batch: `libxml`, `dom`, `simplexml`, `xml`, `xmlreader`, `xmlwriter`, and `xsl` against the validated libxml2/libxslt targets.
...
- [x] Add the second static-library-backed batch: `FFI`, `gmp`, `readline`, and `sodium`.
- [x] Complete the initial database/archive extension batches.
...
- [x] Consolidate the 46-source core and two-source OpenSSL adapter into one `ngtcp2` target and one `out/ngtcp2.lib` archive.
- [x] Compile all 48 selected sources directly without unity, with the merged target depending on OpenSSL and exporting both core and crypto include surfaces.
- [x] Preserve the previously validated 168 core APIs and 54 adapter APIs in the combined source closure and rebuild the merged archive successfully with C11 and `/MD`.
...
- [x] Keep libavif, AOM, and libyuv in one Xmake target and one final static archive named `avif`; do not create preventive AOM/libyuv subtargets.
...
- [x] Build and validate the pre-HEIF baseline at exact 392/392 SDK members, 213/213 `avif*` symbols, x64 architecture, `/MD`, and absence of export directives or `__imp_avif` thunks.
- [x] Fold libheif 1.23.1 and dav1d 1.5.3 into the same `avif` target without merging their source trees. The final archive contains 614 members: the validated libavif/AOM/libyuv closure, all 122 libheif members, and all 99 dav1d members. dav1d matches the SDK reference at 2,131/2,131 `dav1d*` symbols with zero global duplicate symbols across the complete archive; libheif covers all 466/466 SDK DLL exports. PHP/GD AVIF encoding keeps AOM for now because dav1d is decoder-only.

## Completed Target: libjxl

- [x] Replace the PHP SDK binary package with authoritative libjxl 0.11.2 source plus its exact pinned Highway 1.2.0 and skcms revisions, keeping both physically under the libjxl source tree.
- [x] Keep JXL, JXL CMS, skcms, and Highway in one project target/archive named `libjxl`; do not create a separate Highway target. Keep Brotli as the only external target dependency and omit the unused `jxl_threads` runner library.
- [x] Verify Xmake `**.cc` source semantics and use one recursive pattern plus four recursive test/benchmark exclusions; the final unified archive contains the same 139 implementation members represented by SDK `jxl_a + jxl_cms_a + hwy_a`.
- [x] Generate only `version.h`, `jxl_export.h`, and `jxl_cms_export.h` in the target's single `on_prepare`, using static-safe public decoration and no CMake invocation.
- [x] Validate clean and repeated preparation, successful MSVC x64 build, and exact 115/115 `Jxl*` linker-member API coverage against the PHP SDK references with zero differences. Runtime image tests are intentionally skipped.
...
- [x] Build and list `FFI`, `gmp`, `readline`, and `sodium` from `out/php.exe -n -m`.
...
- 2026-08-21 — international-text PHP batch: added builtin `mbstring` and `intl` from 40 C and 73 mixed C/C++ translation units against the validated static Oniguruma and ICU targets. Libmbfl's Windows configuration is copied declaratively. ICU 77's new header-only visibility macro initially exposed C++ standard-library templates inside PHP's deliberate `extern "C"` header blocks; disabling that header-only surface for the extension translation units restored the intended C-only includes without changing upstream source. The complete ZTS CLI build and link passed, both modules appeared in `php.exe -n -m`, and UTF-8 length, mbregex, NFC normalization, supplementary-plane code-point, and ICU 77.1 smoke checks passed.

- 2026-08-21 — security/network/archive PHP batch: added builtin `openssl`, `sockets`, SSL-enabled `ftp`, and `zip` from 16 total extension sources. OpenSSL's required Windows SAPI `applink.c` is copied declaratively into the generated include tree. The first final consumer link exposed a stale libzip probe that incorrectly declared nonexistent `snprintf_s`; its generated configuration now retains the valid MSVC `_snprintf_s` probe only and tracks the Xmake recipe. The complete build passed, all four modules appeared in `php.exe -n -m`, a socket object was created and closed, `ZipArchive` instantiated, FTP functions were present, and the module reported OpenSSL 3.5.7.
...
- 2026-08-21 — second static-library-backed PHP extension batch: linked builtin `FFI`, `gmp`, `readline`, and `sodium` against the validated `libffi`, `mpir`, `wineditline`, and `libsodium` targets with their upstream Windows feature defines. The full build passed and `out/php.exe -n -m` exited normally with all four new modules and every prior module listed.
...
- 2026-08-21 — builtin `calendar` checkpoint: added the exact eight-source Windows manifest, `HAVE_CALENDAR`, and `phpext_calendar_ptr` to the shared ZTS runtime. `php-cli` built successfully and started without crashing with both `calendar` and `ctype` loaded.

- 2026-08-21 — builtin `ctype` checkpoint: added upstream's single `ctype.c` source, `HAVE_CTYPE`, and `phpext_ctype_ptr` to the shared ZTS runtime. The full build passed; CLI, CGI, and console-less CLI loaded `ctype` without crashing, basic predicates passed, and tracing JIT remained active. An unchanged rebuild performed no compilation or linking.
...
- 2026-08-15 — zero-state full dependency validation: a pristine run first exposed and fixed the stale OpenSSL `on_prepare` debug shortcut that left `opensslv.h` ungenerated for libcurl. After the fix, a second true zero-state run removed prepared sources/tools, build output, local Xmake state, HX bootstrap files, MSVC, and the Windows SDK. `xmake prepare` completed in 1m58.80s (an earlier colder clean sample took 2m39.62s), then a plain parallel `xmake` with the 35 completed dependency libraries temporarily enabled by default produced all 35/35 archives with exit code 0 in 4m20.64s. Effective prepare+build time for the successful run was 6m19.44s; using the colder prepare sample gives about 7m00.26s. Normal target enable/default flags were restored afterward.

- 2026-08-15 — ngtcp2 target consolidation: merged the previously separate 46-source core and two-source OpenSSL adapter into one `ngtcp2` target. A targeted build compiled all 48 C11 sources, including `crypto/ossl/ossl.c` and `crypto/shared.c`, and produced the single `out/ngtcp2.lib` archive successfully in 4.938 seconds. The stale old `out/ngtcp2_crypto_ossl.lib` artifact was removed. The only warning was the already-known unrelated OpenSSL `*avx*.asm` unmatched pattern.

- 2026-08-15 — libjxl 0.11.2 validation: replaced the PHP SDK binary input with authoritative libjxl source plus its pinned Highway 1.2.0 (`457c891775a7397bdb0376bb1031e6e027af1c48`) and skcms (`b2e692629c1fb19342517d7fb61f1cf83d075492`) revisions. One project `libjxl` target now combines JXL, JXL CMS, the two baseline skcms sources, and the seven normal Highway sources while Brotli remains an external dependency and `jxl_threads` is omitted because PHP/GD does not use it. Xmake `**.cc` was directly verified to match root plus recursive sources (190 candidates versus 45 for `**/*.cc`); four recursive exclusions reduce the JXL tree to the exact implementation closure. The final archive has 139 members, matching the combined SDK `jxl_a.lib + jxl_cms_a.lib + hwy_a.lib` member count, and its public C API matches all 115/115 `Jxl*` linker-member names with zero differences. Preparation succeeded once from an absent `in/deps/libjxl` tree and again unchanged; the fresh MSVC x64 build passed. Runtime image testing was intentionally skipped.
- 2026-08-15 — unified AVIF/HEIF target validation: `out/avif.lib` now aggregates the physically separate libavif 1.4.2, libheif 1.23.1, AOM 3.14.1, pinned libyuv, and dav1d 1.5.3 trees in one Xmake target, with classic libjpeg retained as a normal dependency for libheif JPEG support. dav1d contributes exactly 99 members, matching the preserved SDK `dav1d_a.lib`, and its 2,131/2,131 `dav1d*` linker-member surface is identical. The complete 614-member archive has zero duplicate global linker-member names. It retains all 466/466 `heif_*` SDK DLL exports; enabling dav1d adds only the expected libavif internal backend entry points beyond the old AOM-only static reference. All inspected members are x64; 547 C/C++ objects select `MSVCRT`, with zero `LIBCMT`, `/EXPORT:`, `__imp_avif`, `__imp_heif`, or `__imp_dav1d` entries. Two unchanged `xmake prepare` runs succeeded. AOM remains enabled because PHP/GD calls libavif's encoder API and dav1d is decoder-only; an AOM encoder-only reduction is a separate future optimization.
- 2026-08-14 — libtiff 4.7.2 validation: replaced the PHP SDK rc2 package with the authoritative final OSGeo 4.7.2 release. The single `libtiff` target builds 43 C sources plus the Windows resource and links against the existing zlib, libjpeg, liblzma, zstd, and unified libwebp targets. `out/libtiff.lib` matches the SDK static archive's 44/44 member manifest and covers all 206/206 SDK DLL exports. The only relevant static linker-member difference is `_TIFFSetDefaultPostDecode`, an internal rc2 helper not exported by the SDK DLL and absent from final 4.7.2. All 43 C objects use `MSVCRT`; there are zero `LIBCMT`, `/EXPORT:`, or TIFF import-thunk entries. The final-release version date `20260627` intentionally replaces rc2's `20260623`; clean preparation and the unchanged repeat both succeeded. Runtime TIFF testing was intentionally skipped.
- 2026-08-14 — libwebp 1.6.0 validation: replaced the PHP SDK package with authoritative `webmproject/libwebp` `v1.6.0`; clean preparation and an unchanged repeat both succeeded. Per project policy, WebP core, demux, mux, and SharpYUV are deliberately collapsed into one `libwebp` target because PHP/GD needs the symbols rather than the SDK's archive partitioning. `out/libwebp.lib` matches the union of `libwebp_a.lib`, `libwebpdemux_a.lib`, and `libwebpmux_a.lib`: 125/125 members and 655/655 relevant linker-member symbols, with 125 `MSVCRT` directives, zero `LIBCMT`, zero `/EXPORT:`, and zero WebP/VP8/SharpYUV import thunks. Runtime image tests were intentionally skipped.
- 2026-08-14 — FreeType 2.14.3 validation: replaced the PHP SDK package with authoritative `freetype/freetype` `VER-2-14-3`; clean preparation and an unchanged repeat both succeeded. The Xmake target reproduces the upstream CMake Windows manifest exactly: 42 C objects plus `ftver.res`. The generated `ftoption.h` is identical to the SDK reference, enabling only optional dynamic HarfBuzz beyond pristine upstream defaults. `out/freetype.lib` matches all 43/43 reference members and 671/671 relevant linker-member symbols; all 42 C objects use `MSVCRT`, with zero `LIBCMT`, `/EXPORT:` directives, FreeType import thunks, or HarfBuzz import thunks. Runtime rendering tests were intentionally skipped.
...
- 2026-08-14 — first upstream runtime-test build at commit `8a33ca3`: Xmake rebuilt the exact static libintl/libiconv chain and reached the final link of `libintl_test.exe`, proving all test-facing public gettext APIs resolve from the direct archives. Link then failed only on unresolved `streq` references from `localename-unsafe.c`, `localcharset.c`, and `localename.c`. The SDK archive does not export or define a standalone `streq`; Gnulib's `stringeq` module supplies it inline through generated `string.h`. Generate that wrapper around UCRT `string.h` with `GNULIB_STRINGEQ=1` and `HAVE_DECL_STREQ=0`, preserving the same 103-source manifest, then retry.
