...
- 2026-08-15 — zero-state full dependency build exposed a stale OpenSSL debug shortcut: the target's `on_prepare` returned immediately, so a pristine source tree had no generated `include/openssl/opensslv.h` and the parallel graph stopped when libcurl reached `curl_sha512_256.c`. Removing the shortcut restored the real OpenSSL generation path; a forced clean OpenSSL codegen/build then completed successfully in 36.46 seconds. Full zero-state dependency timing is being rerun after this fix.
