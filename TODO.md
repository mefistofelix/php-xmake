...
- [ ] Continue the broad `openssl_test` source-glob experiment with preparation temporarily bypassed. Use the deliberately broad `crypto/ec/ecp_nist*.c` exclusion and exclude `**/*md2*.c`, which matches exactly the four-source closure of the default-disabled MD2 feature.
