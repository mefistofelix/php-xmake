...
- Strawberry Perl 5.42.2.1 already contains NASM 2.16.01 at `in/perl/c/bin/nasm.exe`. Point Xmake's assembler toolset at that executable, declare the 39 generated `.asm` files as sources, and pass upstream's `-Ox -DNEAR -g` settings; Xmake owns NASM invocation and archiving.
...
| `openssl` | 39 upstream perlasm scripts selected through `configdata.pm` | x86-64 `.pl` generators and perlasm modules | 39 `.asm` files under `crypto` and `engines` | For each declared output, resolve its unique generator from `%unified_info` and run `perl <generator> nasm <output>` | Xmake invokes bundled NASM directly for the generated files with `-Ox -DNEAR -g`; Perl never assembles or archives | Implemented; build validation pending |
