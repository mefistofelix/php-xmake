...
| `libintl` | Enabled | Static library | Exact PHP SDK manifest: 25 core plus 78 Gnulib C sources and `libintl.rc` | `out/libintl.lib` | GNU message catalogs and locale handling for PHP and dependent libraries | 79/79 SDK DLL exports covered; 103 C objects validated x64 `/MD`; native Windows resource and runtime validation pending |
...
- Match the SDK's final Windows resource member by compiling committed `libintl.rc` with the Windows SDK resource compiler through Xmake, defining version string `1.0` and numeric version `1,0,0,0` directly. This is a native Windows build input, not an external GNU build tool.
