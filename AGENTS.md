...
- Source-level symbol analysis found 39 conflict edges. `alone_decoder.c`, `alone_encoder.c`, `auto_decoder.c`, `block_decoder.c`, and `block_encoder.c` all define `SEQ_CODE`, forming a five-file clique and proving that at least five translation units are required. A five-color partition reaches that lower bound. Its first builds eliminated the source-level collisions but revealed that `lz_decoder.h` and `lz_encoder.h` both define the internal `lzma_lz_options` type. Keep every remaining encoder-header user in the already encoder-compatible fourth exception group: direct user `lz_encoder_mf.c`, plus `lzma_encoder.c` and both `lzma_encoder_optimum_*.c` files through `lzma_encoder_private.h`. Use the wider `lzma_encoder*.c` override because compatible `lzma_encoder_presets.c` is cheaper to include in this group than to exclude from the pattern. The resulting groups contain 55, 11, 3, 3, and 7 sources; no sixth unit is needed. A single recursive encoder/decoder unity grouping is not valid here because same-role XZ files participate in the five-way `SEQ_CODE` conflict, so the conflict-derived exceptions are shorter for this specific setting.
