...
- Generate `config.h`, static `libgnuintl.h`/`libintl.h`, the required `search.h`, `unistd.h`, and `locale.h` Gnulib wrappers, and the small libunistring-compatible generated headers in the target's single callback. On MSVC, generate `locale.h` around the actual UCRT `locale.h` selected from the configured SDK path/version, define Gnulib's POSIX `locale_t` wrapper over native `_locale_t`, and enable the Windows locale-name/setlocale-null modules without running Autoconf, sed, or a Unix shell. The current MSVC C mode obtains `bool` from `<stdbool.h>` rather than as a native C keyword, so leave `HAVE_C_BOOL` undefined while setting `HAVE_STDBOOL_H=1`. Keep generated static headers free of `dllimport`/`dllexport` decoration.
