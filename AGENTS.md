...
- MPIR/GMP: omit it together with optional `ext/gmp`. PHP Windows declares GMP support with `ARG_WITH("gmp", ..., "no")`; its only external requirement is `mpir_a.lib` plus `gmp.h`. No selected core, SAPI, or other extension depends on MPIR. This intentionally drops only PHP's arbitrary-precision integer/GMP API surface.
