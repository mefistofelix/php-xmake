...
| `openssl` | Enabled (current priority) | Static library | Configured `libcrypto` + `libssl` closure under `in/deps/openssl/{crypto,ssl,providers,engines}` plus generated C and x64 NASM | `out/openssl.lib` | TLS and cryptography with built-in default and legacy providers | Initial one-target Xmake integration; build validation pending |
...
- The next default-unit conflicts are smaller cliques within the same 27-group bound: 11 selected `dh_local.h` users, 12 selected `dsa_local.h` users, and six selected `encoder_local.h` users. Retain one user of each header in the default group and distribute the remainder through groups 1–10, 1–11, and 1–5 respectively.
