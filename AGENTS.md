...
| `libavif` | Disabled | Static library | libavif 1.4.2 plus embedded AOM 3.14.1 and pinned libyuv 1924 sources | `out/libavif.lib` | Complete AVIF encode/decode surface in one archive for PHP/GD | Build validated: exact 392/392 SDK member manifest and 213/213 `avif*` symbol surface; x64 `/MD`, no export directives or `__imp_avif` thunks |
...
- Do not create separate `aom` or `libyuv` Xmake targets. The SDK's final `avif_a.lib` is already a merged static archive containing 392 members: 22 libavif objects, all 330 AOM objects, and all 40 selected libyuv objects. Reproduce that same single-dependency/single-target layout as `libavif`.
...
- Generate AOM configuration and RTCD state inside the single `libavif` `on_prepare`. Produce `config/aom_config.h`, `config/aom_config.asm`, `config/aom_config.c`, `config/aom_version.h`, `config/aom_dsp_rtcd.h`, `config/aom_scale_rtcd.h`, `config/av1_rtcd.h`, and the two no-op C files. Run AOM's own `version.pl`/`rtcd.pl` with the already prepared Strawberry Perl; do not invoke CMake or any upstream build system. Generated C files are added with `target:add("files", ...)` after creation inside `on_prepare`; do not use `always_added` to hide an absent generated input.
- Compile all selected libavif, AOM, libyuv, generated C, and NASM inputs into one `out/libavif.lib`. Validate against the 392-member SDK reference, public libavif symbol/API coverage, x64 architecture, `/MD`, and absence of DLL import/export decoration.

### libavif code-generation inventory

| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libavif` | Xmake Lua I/O | AOM defaults + verified SDK Windows x64 configuration | `out/libavif/config/aom_config.h`, `aom_config.asm`, `aom_config.c` | Generate the direct-build AOM config without CMake; add generated C dynamically after creation | Build and SDK surface validation passed |
| `libavif` | AOM `version.pl` via prepared Perl | AOM `CHANGELOG` | `out/libavif/config/aom_version.h` | Preserve upstream version-generation semantics | Build and SDK surface validation passed |
| `libavif` | AOM `rtcd.pl` via prepared Perl | three upstream RTCD definition files + generated AOM config | `out/libavif/config/aom_dsp_rtcd.h`, `aom_scale_rtcd.h`, `av1_rtcd.h` | Generate x86-64 runtime-dispatch headers | Build and SDK surface validation passed |
| `libavif` | Xmake Lua I/O | upstream no-op target requirements | `out/libavif/config/aom_av1_no_op.c`, `aom_dsp_no_op.c` | Create the two empty translation units present in the SDK member manifest; add them dynamically after creation | Build and SDK surface validation passed |
