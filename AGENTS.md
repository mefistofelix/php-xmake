...
| `liblzma` | Enabled (current priority) | Static library | Upstream-default library sources under `in/deps/xz/src/{common,liblzma}` | `out/liblzma.lib` | XZ/LZMA compression for PHP consumers including zip, GD, and fileinfo | One-target build; unity conflict partition under analysis |
...
- Source-level symbol analysis found 39 conflict edges. `alone_decoder.c`, `alone_encoder.c`, `auto_decoder.c`, `block_decoder.c`, and `block_encoder.c` all define `SEQ_CODE`, forming a five-file clique and proving that at least five translation units are required. A five-color partition reaches that lower bound with groups of 60, 11, 3, 3, and 2 sources. Keep the 60-source set in the default unity group and apply four narrow `unity_group` overrides only to the 19 exceptions.
