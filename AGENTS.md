...
- The supported workflow is:
  ...
  ```text
  xmake prepare
  xmake build minilua gen_ir_fold_hash
  xmake
  ```

- `xmake prepare` is run manually before the build and must be idempotent. It downloads and extracts source trees, prebuilt dependencies, and required external tools.
- Build `minilua` and `gen_ir_fold_hash` explicitly after preparation and before the main build. PHP's JIT code generation runs during Xmake's preparation phase and therefore requires those two project-built executables to exist before the main `xmake` invocation.
...
- Imperative configuration and code generation use the single project-level `codegen` rule attached at target level with `add_rules("codegen", {cb = ...})`. The rule owns the preparation hook and invokes the target-specific callback; do not attach code generation to individual source files.
- Incremental code generation uses one marker per participating target at `out/.<target>.codegen`. The shared target-level `codegen` rule checks the marker before invoking the callback and writes it only after the callback succeeds. Deleting `out` invalidates every marker automatically.
- A callback stored in `add_rules` extra configuration does not inherit the rule hook's sandbox directly. The shared `codegen` rule passes only the Xmake facilities callbacks need (`os`, `io`, `path`, `import`, `get_config`); imported modules should still be assigned locally inside the callback.
...
- All target-owned configuration/code generation runs through the target-level `codegen` rule's `on_prepare` hook. Do not use `before_build` or per-file build hooks for generated headers/configuration consumed by multiple sources; those hooks can race normal compilation.
- Every participating target attaches `add_rules("codegen", {cb = ...})`. The rule owns the single `out/.<target>.codegen` convention and writes the marker only after the callback completes successfully.
- Preparation-phase code generation is a barrier before normal compilation, which is why it is the uniform code-generation phase for this project.
- Xmake's preparation phase occurs before project dependency targets are built. `task.run("build", ...)` is not reentrant from either target-level or per-file preparation (`co_group(prepare): already exists`), and Xmake exposes no public `target:build()` API. Spawning nested Xmake processes is intentionally forbidden. Therefore PHP does not depend on `minilua` or `gen_ir_fold_hash` in the Xmake graph; build both explicitly with `xmake build minilua gen_ir_fold_hash` before the main `xmake`, then PHP's `codegen` callback invokes the already-built executables.
- Keep generated source declarations declarative and outside callbacks. If a generated path does not exist when Xmake loads the project, declare the known path explicitly with the appropriate Xmake file option rather than calling `target:add("files", ...)` from code generation.
- Generate C configuration headers and their defines/parameters in the owning target's `codegen` callback. Use Xmake detection APIs for platform properties instead of hard-coding them.
...
| `brotli` | Enabled | Static library | `in/deps/brotli/c/common/*.c`, `in/deps/brotli/c/dec/*.c`, `in/deps/brotli/c/enc/*.c` | `out/brotli.lib` | Complete Brotli common, decoder, and encoder implementation | One target; three compilation units; build validated with `/MD` on MSVC x64 |
| `zstd` | Enabled | Static library | `in/deps/zstd/lib/{common,compress,decompress,dictBuilder,legacy}/*.c` | `out/zstd.lib` | Multithreaded zstd library with level-5 legacy decoding | One main unity group; FastCover and legacy sources isolated; build validated with explicit `/MD` on MSVC x64 |
| `bzip2` | Enabled | Static library | `in/deps/bzip2/*.c` minus upstream programs/tests via `remove_files` | `out/bzip2.lib` | bzip2 compression library for the PHP bz2 extension | Exactly seven library sources in one unity translation unit; declarative selection validated with `/MD` on MSVC x64 |
| `liblzma` | Enabled | Static library | Upstream-default library sources under `in/deps/xz/src/{common,liblzma}` | `out/liblzma.lib` | XZ/LZMA compression for PHP consumers including zip, GD, and fileinfo | One target; five-unit minimum unity partition; build and static-interface validation passed with `/MD` on MSVC x64 |
...
| `libssh2` | Enabled | Static library | 26 sources from `in/deps/libssh2/src/*.c` after seven backend/platform removals | `out/libssh2.lib` | OpenSSL-backed SSH transport required by libcurl/PHP | Build and 137-export static-interface validation passed with `/MD` on MSVC x64 |
| `nghttp2` | Enabled | Static library | All 26 C sources under `in/deps/nghttp2/lib/*.c` | `out/nghttp2.lib` | HTTP/2 transport required by libcurl/PHP | Build and 181-export static-interface validation passed with `/MD` on MSVC x64 |
| `libcurl` | Enabled | Static library | 192 sources under `in/deps/libcurl/lib` after removing `dllmain.c` | `out/libcurl.lib` | PHP curl transport library | Complete dependency build and 100-symbol static-interface validation passed with `/MD` on MSVC x64 |
| `libsodium` | Enabled | Static library | All 141 C files under `in/deps/libsodium/src/libsodium` | `out/libsodium.lib` | Modern cryptography used by PHP sodium | Build and 756-export static-interface validation passed with `/MD` on MSVC x64 |
| `libuv` | Enabled | Static library | 12 common and 25 Windows C sources | `out/libuv.lib` | Event loop and async I/O required by True Async | Build and complete 318-API header-surface validation passed with `/MD` on MSVC x64 |
| `nghttp3` | Enabled | Static library | 31 direct library C sources plus `lib/sfparse/sfparse.c` | `out/nghttp3.lib` | HTTP/3 framing and QPACK required by ngtcp2/PHP | Build and complete 91-API header-surface validation passed with C11 and `/MD` on MSVC x64 |
| `ngtcp2` | Enabled | Static library | All 46 direct C children of `in/deps/ngtcp2/lib` | `out/ngtcp2.lib` | QUIC transport required by PHP HTTP/3 | Build and complete 168-API header-surface validation passed with C11 and `/MD` on MSVC x64 |
| `ngtcp2_crypto_ossl` | Enabled | Static library | `crypto/ossl/ossl.c` and `crypto/shared.c` | `out/ngtcp2_crypto_ossl.lib` | OpenSSL 3.5 QUIC glue required by PHP HTTP/3 | Build and complete 54-API header-surface validation passed with C11 and `/MD` on MSVC x64 |
...
| `oniguruma` | Enabled | Static library | 50 native Windows sources selected from `in/deps/libonig/src/*.c` | `out/oniguruma.lib` | Multibyte regular expressions for PHP mbstring | Build, complete SDK/DLL symbol surface, CRT, architecture, static linkage, and 1,529-case upstream UTF-8 validation passed |
| `sqlite3` | Enabled | Static library | Official SQLite 3.53.2 amalgamation `in/deps/sqlite3/sqlite3.c` | `out/sqlite3.lib` | SQLite core with FTS3/FTS4/FTS5 and column metadata for PHP sqlite3/PDO SQLite | Build, 295/295 SDK DLL API coverage, x64 `/MD`, static linkage, FTS3/4/5, and column-metadata runtime validation passed |
| `libpng` | Enabled | Static library | 15 core C sources plus x64 `intel_init.c` and `filter_sse2_intrinsics.c` | `out/libpng.lib` | PNG codec for PHP GD and other image consumers | Build, 256/256 SDK DLL export coverage, exact 391-symbol `png_*` static surface, x64 `/MD`, SSE2, and upstream runtime validation passed |
| `libjpeg` | Enabled | Static library | libjpeg-turbo 3.1.4.1 traditional libjpeg manifest plus x64 NASM SIMD | `out/libjpeg.lib` | JPEG codec for PHP GD; excludes the separate TurboJPEG API/library | Build and reference symbol validation passed; 129/129 archive members and 371/371 libjpeg/SIMD symbols match |
...
| `libtiff` | Enabled | Static library | 43 C sources plus Windows version resource from libtiff 4.7.2 | `out/libtiff.lib` | TIFF codec for PHP GD with JPEG, zlib, LZMA, Zstd, and WebP support | Build validated: exact 44/44 SDK member manifest and 206/206 SDK DLL API coverage; x64 `/MD`, no static CRT/export/import thunks |
...
| `minilua` | Enabled, non-default | Binary | `in/php-src/ext/opcache/jit/ir/dynasm/minilua.c` | `out/minilua.exe` | Runs DynASM for the PHP JIT IR emitter | Build validated with `/MD` on MSVC x64; built explicitly before the main build |
| `gen_ir_fold_hash` | Enabled, non-default | Binary | `in/php-src/ext/opcache/jit/ir/gen_ir_fold_hash.c` | `out/gen_ir_fold_hash.exe` | Generates the JIT IR fold hash header | Build validated with `/MD` and `IR_TARGET_X64`; built explicitly before the main build |
| `php` | Enabled, non-default | Object prototype | `in/php-src/Zend/zend.c`, `in/php-src/Zend/asm/*_xmm_x86_64_ms_masm.asm` | `out/` | Becomes the static PHP build after dependency, configuration, codegen, and source integration | JIT codegen uses the shared target-level `codegen` rule after the two helper binaries are built explicitly; Windows configuration and full source integration pending |
...
- Preserve the compact target structure and its existing preparation callback, but never bypass that callback in normal builds. A source-clean full-graph build requires OpenSSL Configure/template/perlasm generation before consumers such as libcurl compile; the old temporary fast-skip was removed after it broke a zero-state build.
...
| --- | --- | --- | --- | --- | --- |
| `nghttp2` | Xmake Lua I/O | `lib/includes/nghttp2/nghttp2ver.h.in` | `lib/includes/nghttp2/nghttp2ver.h` | Replace `@PACKAGE_VERSION@` with `1.69.0` and `@PACKAGE_VERSION_NUM@` with `0x014500` in the target's `codegen` rule callback | Build validated |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libsodium` | Xmake file copy | `builds/msvc/version.h` | `src/libsodium/include/sodium/version.h` | Idempotent copy in the target's `codegen` rule callback, matching the official MSVC pre-build event | Build validated |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `nghttp3` | Xmake Lua I/O | `lib/includes/nghttp3/version.h.in` | `lib/includes/nghttp3/version.h` | Replace `@PACKAGE_VERSION@` with `1.18.0` and `@PACKAGE_VERSION_NUM@` with `0x011200` in the target's `codegen` rule callback | Build validated |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `ngtcp2` | Xmake Lua I/O | `lib/includes/ngtcp2/version.h.in` | `lib/includes/ngtcp2/version.h` | Replace `@PACKAGE_VERSION@` with `1.25.0` and `@PACKAGE_VERSION_NUM@` with `0x011900` in the target's `codegen` rule callback | Build validated |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `icu` | Official ICU 77.1 Win64 `genccode.exe` | `source/data/in/icudt77l.dat` | `source/data/in/icudt77l_dat.obj` | Generate a machine-neutral, 16-byte-aligned COFF data object with entry point `icudt77` and no DLL export directive in the target's `codegen` rule callback; merge it directly into `icu.lib` | Build validated |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libiconv` | Xmake Lua I/O | `include/iconv.h.build.in`, `libcharset/include/localcharset.h.build.in`, MSVC/platform configuration | `include/iconv.h`, `libcharset/include/localcharset.h`, `lib/config.h` | Substitute the upstream header templates and write the selected configuration macros in the target's `codegen` rule callback | Build validated |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libintl` | Xmake Lua I/O | `config.h.in`, `libgnuintl.in.h`, Gnulib `*.in.h` configuration templates | `config.h`, static `libgnuintl.h`/`libintl.h`, and required Gnulib compatibility headers | Fixed native-Windows/MSVC substitutions in the target's `codegen` rule callback; no Autoconf, sed, shell, or generated C implementation source | Build and runtime validated |
...
- The Windows configuration step performs no implementation-source generation. It copies committed `include/win32config.h` to `config.h` and substitutes the committed public `include/libxml/xmlversion.h.in`; reproduce those two actions in the target's `codegen` rule callback. Do not execute `configure.js`, CMake, NMake, MSBuild, a solution, or a project file.
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libxslt` | Xmake Lua I/O | `libxslt/xsltconfig.h.in`, `libexslt/exsltconfig.h.in`, pinned SDK feature selection | `libxslt/xsltconfig.h`, `libexslt/exsltconfig.h` | Substitute version and feature placeholders in the target's `codegen` rule callback | Build and runtime validation passed |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `oniguruma` | Xmake file copy | `src/config.h.win64` | `src/config.h` | Copy the committed x64 MSVC configuration in the target's `codegen` rule callback | Build and runtime validation passed |
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libpng` | Xmake Lua I/O | `scripts/pnglibconf.h.prebuilt`, zlib `zlib.h` | `out/libpng/pnglibconf.h` | Use the standard upstream configuration and substitute the pinned `ZLIB_VERNUM` in the target's `codegen` rule callback | Build and runtime validated |
...
- Generate `jconfig.h`, `jconfigint.h`, and `jversion.h` from the committed upstream templates in the target's `codegen` rule callback. Match the SDK public `jconfig.h` exactly on ABI/feature choices: libjpeg v8, libjpeg-turbo 3.1.4.1, arithmetic coding, memory source/destination functions, SIMD, and Windows boolean/INT32 conventions. The private configuration uses MSVC x64 `size_t=8`, `__forceinline`, `__declspec(thread)`, and `intrin.h`.
...
- Generate only `out/freetype/include/freetype/config/ftoption.h` in the target's `codegen` rule callback by copying the upstream header and enabling those two verified macros. The Windows `ftconfig.h` and committed `ftmodule.h` are already the correct upstream files; the SDK `ftmodule.h` is byte-equivalent apart from line endings.
...
- Generate only the three public build outputs needed by the source/API: `version.h`, `jxl_export.h`, and `jxl_cms_export.h` under `out/libjxl/include/jxl` in the target's `codegen` callback; do not invoke CMake.

### libjxl code-generation inventory

| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libjxl` | Xmake Lua I/O | `lib/jxl/version.h.in` | `out/libjxl/include/jxl/version.h` | Substitute version 0.11.2 in the target's `codegen` rule callback | Build and SDK surface validation passed |
...
- Generate only `libheif/heif_version.h` in the `avif` target's `codegen` rule callback; public consumers receive `LIBHEIF_STATIC_BUILD` so Windows headers do not emit `dllimport` decoration.
...
| `avif` | Xmake Lua I/O | 13 dav1d `*_tmpl.c` sources | 26 wrapper C files under `out/avif/dav1d` | Generate one translation unit for each 8- and 16-bit specialization; all wrapper paths are declared statically with `add_files` outside the callback | Exact 99-member dav1d manifest validation passed |
...
- Generate only the frontend configuration inputs actually required by this closure: render `pg_config.h` and `pg_config_ext.h` from their committed templates, copy `src/include/port/win32.h` as `pg_config_os.h`, and write `pg_config_paths.h` in the target's `codegen` rule callback. No Perl generator, `kwlist_d.h`, backend/catalog generation, Visual Studio build, or Meson build is required.
...
- Generate `portable.h`, `lber_types.h`, `ldap_features.h`, `ldap_config.h`, and the small library `version.c` input in the target's `codegen` rule callback. Match the SDK client configuration: x64 64-bit BER tag/length types, NT threads, Winsock2/IPv6, OpenSSL TLS, Cyrus SASL, thread safety, and OpenLDAP 2.6.13.
...
- Generate `zipconf.h`, the Windows `config.h`, and `zip_err_str.c` in the target's `codegen` rule callback. `zip_err_str.c` is derived from the error definitions/comments in committed `zip.h` and `zipint.h`, matching upstream `GenerateZipErrorStrings.cmake`; declare the generated source statically with `add_files` outside the callback. Do not invoke CMake.
...
| Owner | Tool | Input | Output | Handling | Status |
| --- | --- | --- | --- | --- | --- |
| `libzip` | Xmake Lua I/O | verified Windows/x64 configuration + `zipconf.h.in` semantics | `out/libzip/config.h`, `out/libzip/zipconf.h` | Emit the selected Windows static configuration directly in the target's `codegen` rule callback | Build validated |
...
- The native pre-build generates four configuration/public inputs. For the generic build, `config.h` is the committed `build.vc/cfg.h` preceded only by the generated comment; no `HAVE_NATIVE_*` assembly macros are added. Generate identical static `mpir.h` and `gmp.h` from `gmp-h.in` using the semantics of `gen_mpir_h.bat`: inject the Windows 32/64-bit limb block at `GMP_NAIL_BITS` and omit unresolved configure placeholders. Copy `mpn/generic/gmp-mparam.h`, and generate `longlong.h` by concatenating `longlong_pre.h`, `mpn/x86_64w/longlong_inc.h`, and `longlong_post.h`. Own all four actions in the target's `codegen` callback; do not invoke MSBuild or the upstream batch build.
...
All paths below are relative to `in/php-src` unless prefixed with `out/`. PHP code generation runs in PHP's target-level `codegen` callback during the rule's preparation phase. `minilua` and `gen_ir_fold_hash` are explicit pre-build prerequisites, not PHP target dependencies, so that callback can invoke their already-built executables deterministically.
...
| `php` `codegen` | `minilua` target and DynASM | `ext/opcache/jit/ir/ir_x86.dasc` | `ext/opcache/jit/ir/ir_emit_x86.h` | `out/minilua.exe ext/opcache/jit/ir/dynasm/dynasm.lua -D X64=1 -D X64WIN=1 -D WIN=1 -o ext/opcache/jit/ir/ir_emit_x86.h ext/opcache/jit/ir/ir_x86.dasc`; build `minilua` explicitly before the main `xmake` | Wired; explicit prerequisite enforced |
| `php` `codegen` | `gen_ir_fold_hash` target | `ext/opcache/jit/ir/ir_fold.h` | `ext/opcache/jit/ir/ir_fold_hash.h` | `out/gen_ir_fold_hash.exe < ext/opcache/jit/ir/ir_fold.h > ext/opcache/jit/ir/ir_fold_hash.h`; build it explicitly before the main `xmake`; generator uses `IR_TARGET_X64` | Wired; explicit prerequisite enforced |
