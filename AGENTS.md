...
| `bzip2` | Enabled (focused revalidation) | Static library | `in/deps/bzip2/*.c` minus upstream programs/tests via `remove_files` | `out/bzip2.lib` | bzip2 compression library for the PHP bz2 extension | All seven library sources in one unity translation unit; glob/removal revalidation pending |
| `liblzma` | Disabled temporarily | Static library | Upstream-default library sources under `in/deps/xz/src/{common,liblzma}` | `out/liblzma.lib` | XZ/LZMA compression for PHP consumers including zip, GD, and fileinfo | One maximal unity group; build validation pending after bzip2 revalidation |
