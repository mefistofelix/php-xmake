...
- The final `out/libpq.lib` built successfully from official PostgreSQL `REL_16_14`. It contains all 187/187 exports from `dll_compare/libpq.dll` with zero missing names and 400 public linker-member symbols total. Archive-level inspection reports 54 x64 members, 54 `MSVCRT` directives, zero `LIBCMT`, zero embedded `/EXPORT:` directives, and zero libpq-family import thunks. Extra internal/support symbols in the static archive are expected and are not a validation failure.
