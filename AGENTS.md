...
- The native pre-build generates four configuration/public inputs. For the generic build, `config.h` is the committed `build.vc/cfg.h` preceded only by the generated comment; no `HAVE_NATIVE_*` assembly macros are added. Generate identical static `mpir.h` and `gmp.h` from `gmp-h.in` using the semantics of `gen_mpir_h.bat`: inject the Windows 32/64-bit limb block at `GMP_NAIL_BITS` and omit unresolved configure placeholders. Copy `mpn/generic/gmp-mparam.h`, and generate `longlong.h` by concatenating `longlong_pre.h`, `mpn/x86_64w/longlong_inc.h`, and `longlong_post.h`. Own all four actions in the target's single `on_prepare`; do not invoke MSBuild or the upstream batch build.
