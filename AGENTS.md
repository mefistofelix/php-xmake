...
- Generate a minimal `lib/config.h` for the selected sources: default extra encodings disabled, no Unix `langinfo` API, working MSVC `mbrtowc`, `mbsinit`, and `wcrtomb`, an empty `ICONV_CONST`, and little-endian byte order. Define `BUILDING_LIBICONV` and `BUILDING_LIBCHARSET` privately as upstream does. Suppress MSVC's secure-CRT warning for the upstream bounded `sprintf` calls. On `iconv.c` only, suppress C4311 for the generated gperf tables' deliberate null-base member-offset casts and C4090 where `qsort(void *)` reorders an array of `const char *` without modifying the strings.
